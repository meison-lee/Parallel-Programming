...
int main(int argc, char *argv[]){
  ...
  if (recvNum){
    if (recvNum + 2 == firstPrime){
      Local_count++;
    }
  }
  printf("id = %d, count = %d\n", id, Local_count);
  MPI_Reduce(&Local_count, &globalCount, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);

  elapsed_time += MPI_Wtime();

  if (!id){
    printf("global Local_count = %d\n", globalCount);
    printf("time = %f\n", elapsed_time);
  }
