...
int main(int argc, char* argv[]) {
    ...
    double communicatorTime;
    double readcheckTime;
    double readvector;
    double ifTIme;
    ...
    readcheckTime = -MPI_Wtime();
    read_checkerboard_matrix("matrix.txt", (void*)&a,
    ...
    readvector = -MPI_Wtime();
    if (grid_coords[1] == 0) {
        read_block_vector("vector.txt", (void*)&b, mpitype, &nprime, col_comm);
    }
    readvector += MPI_Wtime();
    ...
    if (!id) {
        ...
        printf("commu:%lf readch:%lf readvec:%lf iftime:%lf\n", communicatorTime, readcheckTime, readvector, ifTIme);
    }
    MPI_Finalize();
    return 0;
}
